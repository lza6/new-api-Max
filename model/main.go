...
func createRootAccountIfNeed() error {
	...
	if err := DB.First(&user).Error; err != nil {
		common.SysLog("no user exists, create a root user for you: username is root, please change the default password immediately")
...
func InitDB() (err error) {
	...
	if err == nil {
		...
		sqlDB.SetMaxIdleConns(common.GetEnvOrDefault("SQL_MAX_IDLE_CONNS", 16))
		sqlDB.SetMaxOpenConns(common.GetEnvOrDefault("SQL_MAX_OPEN_CONNS", 64))
...
PRIMARY KEY (` + "`id`" + `)
	...
	required := []sqliteColumnDef{
		...
		{Name: "concurrency_limit", DDL: "`concurrency_limit` int DEFAULT 0"},
		{Name: "rpm_limit", DDL: "`rpm_limit` int DEFAULT 0"},
		{Name: "models", DDL: "`models` text DEFAULT ''"},
