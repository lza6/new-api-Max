...
type Token struct {
	...
	// RateLimit T5 单密钥限速：JSON 字符串，形如 {"rpm":60,"qbs":5,"concurrency":2}。
	// rpm=每分钟最大请求数；qbs=每秒配额；concurrency=同时进行中的最大请求数。
	// 空 = 不限制（沿用分组/全局限流）。
	RateLimit string         `json:"rate_limit" gorm:"type:text"`
	DeletedAt gorm.DeletedAt `gorm:"index"`
}

// TokenRateLimitConfig 单密钥限速配置（T5）。
...
func (token *Token) GetRateLimitConfig() TokenRateLimitConfig {
	if token == nil || token.RateLimit == "" {
		return TokenRateLimitConfig{}
	}
	var cfg TokenRateLimitConfig
	if err := common.UnmarshalJsonStr(token.RateLimit, &cfg); err != nil {
		return TokenRateLimitConfig{}
	}
	return cfg
}
...
func (token *Token) Update() (err error) {
	...
	return DB.Model(token).Select("name", "status", "expired_time", "remain_quota", "unlimited_quota",
		"model_limits_enabled", "model_limits", "allow_ips", "group", "cross_group_retry", "auto_groups", "rate_limit").Updates(token).Error
}
