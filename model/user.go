...
// 用户注册来源（User.Source）取值。空字符串 = 历史存量用户（未打标）。
const (
	UserSourcePassword = "password" // 用户名/邮箱 + 密码注册
	UserSourceAdmin    = "admin"    // 管理员手动创建
...
type User struct {
	...
	// Source 记录用户注册来源：password / admin / github / discord / wechat /
	// telegram / linuxdo / oidc / 自定义 OAuth provider 名。空 = 历史存量用户。
	Source           string                     `json:"source" gorm:"type:varchar(64);default:'';column:source;index"`
