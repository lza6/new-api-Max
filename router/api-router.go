...
import (
	...
	_ "github.com/lza6/new-api-Max/oauth"
...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		// 公共只读聚合（无 PII）：首页/模型广场公开展示，未登录可访问。
		// 不能用登录型限流（SearchRateLimit 对匿名请求 401）——保持无认证。
		apiRouter.GET("/site/stats", controller.GetSiteOverview)
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			//userRoute.POST("/tokenlog", middleware.LoginRateLimit(), controller.TokenLog)
			...
			selfRoute.Use(middleware.DisableCache(), middleware.UserAuth())
			{
				...
				selfRoute.GET("/profile/insights", middleware.DisableCache(), controller.GetUserProfile)
			...
			{
				// P1 管理员重置所有签到（福利重置）
				adminRoute.POST("/checkins/reset", controller.ResetAllCheckins)
		...
		subscriptionAdminRoute.Use(middleware.AdminAuth())
		{
			...
			subscriptionAdminRoute.GET("/logs", controller.AdminListSubscriptionLogs)
			...
			subscriptionAdminRoute.PATCH("/user_subscriptions/:id/tier", controller.AdminSetUserSubscriptionTier)
		...
		taskRoute := apiRouter.Group("/task")
		{
			...
			taskRoute.GET("/:task_id/events", middleware.UserAuth(), controller.TaskEventsSSE)
		...
		modelsRoute.Use(middleware.AdminAuth())
		{
			...
			modelsRoute.POST("/groups", controller.UpdateModelGroups)
